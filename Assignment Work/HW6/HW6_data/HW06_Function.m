...
function [T1, T2, T3, err_Fx_3, err_Fy_3, err_T3] = HW06_Function(theta1,theta2,thetadot1,thetadot2,thetadotdot1,thetadotdot2,GRF_hori,GRF_vert)
...
Fx_3_hg =  -m3*r3*(sin(theta3).*thetadotdot3 + cos(theta3).*thetadot3.^2);
...
%Fy_3_hg = m3*g + m3*L1.*((cos(theta1).*thetadotdot1) - (sin(theta1).*thetadot1.^2)) ...
 ...
 Fy_3_hg = m3*g + m3*r3*(cos(theta3).*thetadotdot3 - sin(theta3).*thetadot3.^2);   
